/**
 * 🔍 Agent Inspector V3 — 미슐랭 복면 심사관
 * 
 * ⚠️ 설계 원칙:
 * "고장 났는가?"가 아닌, "이 보고서에 5000만원을 걸 수 있는가?"를 검증합니다.
 * 
 * Phase 1-2: 파이프라인 & 보고서 완결성
 * Phase 3: 데이터 파이프라인 건강 진단
 * Phase 4: AI 할루시네이션 팩트체크
 * Phase 6: 논리적 모순 및 이상치 탐지
 * Phase 7: AI 인사이트 퀄리티 딥체크
 * Phase 8: 프리미엄 데이터 정합성 검사
 * Phase 9: 🎭 AI 복면 심사 (고객 관점 품질 채점) ← V3 신설
 * Phase 10: 🔀 교차 비교 심사 (구 내 일관성 검증) ← V3 신설
 */
...
export async function inspectAddress(address, radius = 500, targetCategory = '카페') {
    ...
    try {
        ...
        // Phase 9: 🎭 AI 복면 심사 (V3 — 고객 관점 품질 채점)
        const michelinReview = await runMichelinReview(pipelineResult, address, targetCategory);
        if (michelinReview) {
            issues.push(...michelinReview.issues);
            pipelineResult.michelinReview = michelinReview.review;
        }
    ...
    return {
        ...
        analysisSnapshot: pipelineResult ? {
            ...
            michelinScore: pipelineResult.michelinReview?.score || null,
            michelinVerdict: pipelineResult.michelinReview?.verdict || null
        } : null
    };
}
...
export async function inspectDistrict(districtName, addresses, categories = null) {
    ...
    // Phase 10: 🔀 교차 비교 심사 (V3 — 구 내 일관성 검증)
    const crossReview = await runCrossValidation(results, districtName);

    const totalIssues = results.reduce((sum, r) => sum + (r.totalIssues || 0), 0) + (crossReview?.issues?.length || 0);
    ...
    return {
        ...
        summary: {
            ...
            high: results.reduce((sum, r) => sum + (r.issuesBySeverity?.HIGH || 0), 0) + (crossReview?.issues?.filter(i => i.severity === 'HIGH')?.length || 0),
            medium: results.reduce((sum, r) => sum + (r.issuesBySeverity?.MEDIUM || 0), 0) + (crossReview?.issues?.filter(i => i.severity === 'MEDIUM')?.length || 0),
            low: results.reduce((sum, r) => sum + (r.issuesBySeverity?.LOW || 0), 0) + (crossReview?.issues?.filter(i => i.severity === 'LOW')?.length || 0)
        },
        pipelineHealth: pipelineHealth.length === 0 ? 'ALL_HEALTHY' : `${pipelineHealth.length}건 이상`,
        crossReview: crossReview?.narrative || null,
        michelinSummary: results.filter(r => r.analysisSnapshot?.michelinScore).map(r => ({
            address: r.address?.split(' ').slice(-2).join(' '),
            score: r.analysisSnapshot.michelinScore,
            verdict: r.analysisSnapshot.michelinVerdict
        })),
        results
    };
}
...
const CATEGORY_ALIASES = {
    '음식점': ['외식·음료', '한식', '일식·중식', '양식·레스토랑', '분식·간식', '패스트푸드', '간식·분식'],
    '미용실': ['뷰티·미용', '뷰티·화장품'],
    '카페':   ['카페·음료'],
    '편의점': ['편의점·슈퍼'],
    '병원':   ['병원·의료', '약국·의료'],
...
function matchesCategoryAlias(categoryName, targetCategory) {
    // 1. 직접 부분 매칭
    if (categoryName.includes(targetCategory) || targetCategory.includes(categoryName)) return true;
    // 2. alias 맵 매칭
    const aliases = CATEGORY_ALIASES[targetCategory];
    if (aliases && aliases.some(alias => categoryName.includes(alias) || alias.includes(categoryName))) return true;
    return false;
}
...
// ─────────────────────────────────────────────
// Phase 9: 🎭 AI 복면 심사 (V3 — 미슐랭 고객 관점 채점)
// ─────────────────────────────────────────────

async function runMichelinReview(result, address, targetCategory) {
    try {
        const { analysis, aiComments, transitInfo, seoulData, demographics } = result;
        if (!analysis || !aiComments) return null;

        const briefAddress = address.split(' ').slice(-2).join(' ');
        const prompt = `당신은 수억원 투자를 앞둔 예비 창업자입니다. '${targetCategory}' 업종으로 '${briefAddress}' 근처 창업을 검토 중입니다.
아래는 상권 분석 서비스가 제공한 보고서입니다. 이 보고서만으로 투자 의사결정이 가능한지 냉정하게 평가하세요.

[분석 결과]
- 종합점수: ${analysis.overallScore}점 (${analysis.grade.grade}등급)
- 반경 내 업소: ${analysis.totalStores}개
- 상위 업종: ${analysis.categorySummary.slice(0,5).map(c => `${c.name}(${c.count}개)`).join(', ')}
- 프랜차이즈 비율: ${analysis.franchiseAnalysis.franchiseRatio}%
${transitInfo ? `- 교통 접근성: ${transitInfo.score}점 (지하철 ${transitInfo.totalSubways || 0}개, 버스 ${transitInfo.totalBusStops || 0}개)` : ''}
${seoulData?.store ? `- 분기 추정매출: ${seoulData.store.totalSales ? Math.floor(seoulData.store.totalSales/100000000) + '억원' : '데이터 없음'}` : ''}

[AI 컨설턴트 코멘트]
${aiComments.overview || '없음'}
강점: ${(aiComments.strengths || []).join(' / ')}
약점: ${(aiComments.weaknesses || []).join(' / ')}

아래 JSON만 출력하세요:
{
  "score": 1~5 (1=의사결정 불가, 5=즉시 투자 결정 가능),
  "verdict": "한줄 판정 (30자 이내)",
  "missingPerspectives": ["이 보고서에서 빠진 관점 (최대 2개)"],
  "contradictions": ["데이터와 AI 코멘트 간 모순 (있으면)"]
}`;

        const raw = await askGemini(prompt, null, '복면 고객 심사관. JSON만 반환.');
        ...
        if (review.score <= 2) {
            issues.push({
                phase: 'Phase 9: AI 복면 심사',
                severity: 'HIGH',
                type: 'MICHELIN_LOW_SCORE',
                description: `복면 심사 ${review.score}/5점: "${review.verdict}" — ${briefAddress}`,
                suggestion: review.missingPerspectives?.join(', ') || '보고서 품질 전반 점검'
            });
        } else if (review.score === 3) {
            issues.push({
                phase: 'Phase 9: AI 복면 심사',
                severity: 'MEDIUM',
                type: 'MICHELIN_MEDIOCRE',
                description: `복면 심사 ${review.score}/5점: "${review.verdict}" — ${briefAddress}`,
                suggestion: review.missingPerspectives?.join(', ') || '참고사항'
            });
        }

        if (review.contradictions && review.contradictions.length > 0) {
            issues.push({
                phase: 'Phase 9: AI 복면 심사',
                severity: 'MEDIUM',
                type: 'MICHELIN_CONTRADICTION',
                description: `AI-데이터 모순 감지: ${review.contradictions[0]} — ${briefAddress}`,
                suggestion: 'AI 프롬프트 또는 데이터 파이프라인 점검'
            });
        }

        console.log(`   🎭 복면 심사: ${review.score}/5점 — "${review.verdict}"`);
        return { review, issues };

    } catch (e) {
        console.warn(`   ⚠️ Phase 9 복면 심사 실패: ${e.message}`);
        return null;
    }
}
...
async function runCrossValidation(results, districtName) {
    try {
        const validResults = results.filter(r => r.analysisSnapshot);
        if (validResults.length < 2) return null;

        const issues = [];

        // 규칙 기반 체크 (API 호출 없음)
        const scores = validResults.map(r => r.analysisSnapshot.overallScore);
        const maxScore = Math.max(...scores);
        const minScore = Math.min(...scores);
        const spread = maxScore - minScore;

        const maxResult = validResults.find(r => r.analysisSnapshot.overallScore === maxScore);
        const minResult = validResults.find(r => r.analysisSnapshot.overallScore === minScore);

        // 업소 30개 미만인데 A/S 등급
        validResults.forEach(r => {
            const snap = r.analysisSnapshot;
            if (snap.storeCount < 30 && ['S', 'A'].includes(snap.grade)) {
                issues.push({
                    phase: 'Phase 10: 교차 비교',
                    severity: 'HIGH',
                    type: 'INFLATED_GRADE',
                    description: `업소 ${snap.storeCount}개 상권에 ${snap.grade}등급(${snap.overallScore}점) 부여 — ${r.address?.split(' ').slice(-2).join(' ')}`,
                    suggestion: '소규모 상권 등급 산출 로직 점검 (데이터 부족 시 등급 하향 필요)'
                });
            }
        });

        // Gemini 교차 심사 (1회 호출)
        const summaryText = validResults.map(r => {
            const snap = r.analysisSnapshot;
            const addr = r.address?.split(' ').slice(-2).join(' ');
            const michelin = snap.michelinScore ? ` [복면심사 ${snap.michelinScore}/5]` : '';
            return `- ${addr}: ${snap.overallScore}점 ${snap.grade}등급, 업소 ${snap.storeCount}개${michelin}`;
        }).join('\n');

        const crossPrompt = `${districtName}의 6개 상권 분석 결과입니다. 서비스 품질 관점에서 전체를 평가하세요.

${summaryText}

아래 JSON만 출력하세요:
{
  "consistency": "일관적/불균형" (채점 기준 일관성),
  "concern": "서비스 품질 관점에서 가장 우려되는 점 1가지 (50자 이내)",
  "narrative": "${districtName} 전체 종합 평가 2~3문장 (순수 텍스트)"
}`;

        const raw = await askGemini(crossPrompt, null, '상권분석 서비스 품질 감사관. JSON만 반환.');
        const cleaned = raw.replace(/```json\s*/gi, '').replace(/```\s*/g, '').trim();
        const crossResult = JSON.parse(cleaned);

        if (crossResult.consistency === '불균형') {
            issues.push({
                phase: 'Phase 10: 교차 비교',
                severity: 'MEDIUM',
                type: 'CROSS_INCONSISTENCY',
                description: `채점 일관성 불균형: ${crossResult.concern}`,
                suggestion: 'analyzer.js 등급/점수 산출 기준 점검'
            });
        }

        console.log(`   🔀 교차 비교: ${crossResult.consistency} — "${crossResult.concern}"`);
        return { narrative: crossResult.narrative, issues };

    } catch (e) {
        console.warn(`   ⚠️ Phase 10 교차 비교 실패: ${e.message}`);
        return null;
    }
}
