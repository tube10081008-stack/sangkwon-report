...
4. **타겟 업종이 상위 업종에 없을 때의 해석**: [관심 타겟 업종]이 [주요 업종 탑 3]에 포함되지 않으면, 단순히 블루오션이라 단정하지 마세요. 경쟁자 부재가 수요 부재일 가능성도 반드시 언급하고, 상권 성격과 타겟 업종의 시너지를 데이터 기반으로 판단하세요.
5. **지표 간 일관성**: 상권 활성도(전체 업소 밀도)와 특정 업종의 잠재 수요는 별개 지표입니다. 같은 답변에서 활성도가 높아 유리하다와 잠재 수요가 부족하다를 모순되게 병치하지 마세요. 수준을 분리하여 해석하세요.
